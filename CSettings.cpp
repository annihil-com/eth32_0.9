...
void CSettings::Load(const char *filename)
{
	...
	for (int i=0 ; i<numSettings ; i++)
	{
		...
		GetPrivateProfileString("Main Settings", setting->name, setting->defaultValue, value, sizeof(value), filename);
		...
		case SETTING_VEC3:
		{
			...
			sscanf_s(value, "%f %f %f", &v[0], &v[1], &v[2]);
			break;
		}
		case SETTING_VEC4:
		{
			float *v = (float*)setting->target;
			sscanf_s(value, "%f %f %f %f", &v[0], &v[1], &v[2], &v[3]);
			break;
		}
		case SETTING_BYTE3:
		{
		...
		case SETTING_STRING:
		{
			strncpy_s((char *)setting->target, MAX_STRING_CHARS, value, _TRUNCATE);
			break;
		}
		default:
#ifdef ETH32_DEBUG
...
void CSettings::Save(const char *filename)
{
	...
	for (int i=0 ; i<numSettings ; i++)
	{
		...
		{
		case SETTING_INT:
			sprintf_s(value,sizeof(value), "%i", *(int*)setting->target);
			break;
		case SETTING_FLOAT:
			sprintf_s(value,sizeof(value), "%.3f", *(float*)setting->target);
			break;
		case SETTING_BOOL:
			sprintf_s(value,sizeof(value), "%s", *(bool*)setting->target ? "true" : "false");
			break;
		case SETTING_VEC3:
		{
			float *v = (float*)setting->target;
			sprintf_s(value,sizeof(value), "%.3f %.3f %.3f", v[0], v[1], v[2]);
			break;
		}
		case SETTING_VEC4:
		{
			float *v = (float*)setting->target;
			sprintf_s(value,sizeof(value), "%.3f %.3f %.3f %.3f", v[0], v[1], v[2], v[3]);
			break;
		}
		case SETTING_BYTE3:
		{
			BYTE *b = (BYTE*)setting->target;
			sprintf_s(value,sizeof(value), "%i %i %i", b[0], b[1], b[2]);
			break;
		}
		case SETTING_STRING: // using pointer to pointer
		{  
			strncpy_s(value, sizeof(value), (char *)setting->target, _TRUNCATE);
			break;
		}
		default:
#ifdef ETH32_DEBUG
		...
		WritePrivateProfileString("Main Settings", setting->name, value, filename);
	}

	if(!eth32.settings.defaultSoundsLoaded)
	{
		...
		for(int t = 0; t < SOUND_LEVEL_MAX; t++)
		{
			...
			{
				sprintf_s(buf1, sizeof(buf1), "sound%i%i", t, s);
				sprintf_s(buf2, sizeof(buf2), "%s", defaultSounds[soundName]);
			
				WritePrivateProfileString("Sound Settings", buf1, buf2, filename);
				soundName++;
			}

		}
		WritePrivateProfileString("Main Settings", "loaddefaultsounds", "true", filename);
	}
}
...
void CSettings::ConsoleCommand(void)
{
	...
	if (Syscall.UI_Argc() == 2){
		...
		if (found == 1) {
			...
			strcpy_s(value, sizeof(value), Syscall.UI_Argv(2));
			int argnum = 3;
			while (argnum < Syscall.UI_Argc()) {
				strcat_s(value, sizeof(value), " ");
				strcat_s(value, sizeof(value), Syscall.UI_Argv(argnum));
				argnum++;
			}
			...
			switch (setting->type) {
				case SETTING_INT: 
				{
					if (setting->values[0].text) {
						...
						{
							if (!_strcmpi(value, setting->values[i].text)) { // found a match
				...
				case SETTING_FLOAT:
					*(float*)(setting->target) = atof(value);
					break;
				case SETTING_BOOL:
					*(bool*)(setting->target) = (_stricmp(value, "on") == 0);
					break;
				case SETTING_VEC3:
				{
					float *v = (float*)setting->target;
...
void CSettings::Toggle(void)
{
	...
	for (int i=0 ; i<numSettings ; i++) 
	{
		...
		if (!_strcmpi(varname, setting->name)) {
			char buffer[256];

			if (setting->type == SETTING_BOOL) {
				*(bool*)setting->target = !(*(bool*)setting->target);
				sprintf_s(buffer, sizeof(buffer), "^otoggle: ^d%s ^ois now ^d%s", setting->name, (*(bool*)setting->target)?"on":"off");
				...
				*(int*)setting->target = cur;
				...
				sprintf_s(buffer, sizeof(buffer), "^otoggle: ^d%s ^ois now ^d%s", setting->name, setting->values[cur-(int)setting->min].text);
				Gui.CenterPrint(buffer, 1);
				return;
			} 
			else {
				sprintf_s(buffer, sizeof(buffer), "^d%s ^ocannot be toggled", setting->name);
				Gui.CenterPrint(buffer, 1);
				return;
			}
		}
...
void CSettings::PrintVarDescription(const settingdef_t *var)
{
	...
	case SETTING_INT: 
	{
		int value = *(int*)var->target;
		if (var->values[0].text && (value >= (int)var->min) && (value <= (int)var->max))
			strcpy_s(args, sizeof(args), var->values[value].text);
		else
			sprintf_s(args, sizeof(args), "%i", value);
		break;
	}
	case SETTING_FLOAT:
	{
		float value = *(float*)var->target;
		sprintf_s(args, sizeof(args), "%.3f", value);
		break;
	}
	case SETTING_BOOL:
		strcpy_s(args, sizeof(args), *(bool*)var->target ? "on" : "off");
		break;
	case SETTING_VEC3:
	{
		float *v3 = (float*)var->target;
		sprintf_s(args, sizeof(args), "%0.3f %0.3f %0.3f", v3[0], v3[1], v3[2]);
		break;
	}
	case SETTING_VEC4:
	{
		float *v4 = (float*)var->target;
		sprintf_s(args, sizeof(args), "%0.3f %0.3f %0.3f %0.3f", v4[0], v4[1], v4[2], v4[3]);
		break;
	}
	case SETTING_BYTE3:
	{
		BYTE *b3 = (BYTE*)var->target;
		sprintf_s(args, sizeof(args), "%i %i %i", b3[0], b3[1], b3[2]);
		break;
	}
	case SETTING_STRING:
		sprintf_s(args, sizeof(args), "\"%s\"", (char*)var->target);
		break;
	default:
		args[0] = 0;
		break;
	...
	case SETTING_INT:
		if (var->values[0].text) {
			char temparg[256];
			sprintf_s(temparg, sizeof(temparg), "^9%s", var->values[0].text);
			strcpy_s(args, sizeof(args), temparg);
			int valuenum = 1;
			while (var->values[valuenum].text) {
				sprintf_s(temparg, sizeof(temparg), "^2|^9%s", var->values[valuenum].text);			
				strcat_s(args, sizeof(args), temparg);
				valuenum++;
			}
		}
		else
			sprintf_s(args, sizeof(args), "^9%i^2-^9%i", (int)var->min, (int)var->max);

		break;
	case SETTING_FLOAT:
		sprintf_s(args, sizeof(args), "^9%.3f^2-^9%.3f", var->min, var->max);
		break;
	case SETTING_BOOL:
		strcpy_s(args, sizeof(args), "^9on^2|^9off");
		break;
	case SETTING_VEC3:
		strcpy_s(args, sizeof(args), "^90.0^2-^91.0 ^90.0^2-^91.0 ^90.0^2-^91.0");
		break;
	case SETTING_VEC4:
		strcpy_s(args, sizeof(args), "^90.0^2-^91.0 ^90.0^2-^91.0 ^90.0^2-^91.0 ^90.0^2-^91.0");
		break;
	case SETTING_BYTE3:
		strcpy_s(args, sizeof(args), "^90^2-^9255 ^90^2-^9255 ^90^2-^9255");
		break;
	case SETTING_STRING:
		strcpy_s(args, sizeof(args), "^9\"string\"");
		break;
	default:
		args[0] = 0;
		break;
...
void CSettings::PrintValue(const settingdef_t *var, char *val)
{
	...
	strncpy_s(val, sizeof(args), args, _TRUNCATE);
}
