...
extern const char *autoPointText[];
extern const char *coopTypeText[];

/* enum and struct for advanced trace */
typedef enum {
	TRACE_CENTER,				// trace center point only
 	TRACE_RANDOM_VOLUME,		// generate random points within box volume
   	TRACE_RANDOM_SURFACE,		// generate random points on the side(s) of the box that is(are) facing us
	TRACE_CAPSULE_VOLUME,		// fill a capsule with random points
	TRACE_CAPSULE_SURFACE,		// fill the surface of a capsule with random points
...
extern const char *traceTypeText[];
...
extern const char *multiHeadText[];
...
extern const char *multiBodyText[];
...
public:
	...
	void DrawBodyBox( int clientNum, bool axes );
...
private:
	...
	bool rifleEndPos(vec3_t forward, vec3_t origin);
	bool traceBox( vec3_t boxOrigin, float dist, vec3_t size, vec3_t trOrigin, int skipEnt, player_t *player, boxtrace_t trType,  vec3_t visible, int maxTraces );
