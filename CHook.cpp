...
void eth32_FS_PureServerSetLoadedPaks(const char *pakSums, const char *pakNames)
{
	...
	strncpy_s(goodChecksumsPak, sizeof(goodChecksumsPak), pakSums, sizeof(goodChecksumsPak));
	...
	{
		sprintf_s(fakePakSums, sizeof(fakePakSums), "%s%i ", pakSums, eth32Pack->checksum);
		sprintf_s(fakePakNames, sizeof(fakePakNames), "%s %s/%s", pakNames, eth32Pack->pakGamename, eth32Pack->pakBasename);
...
{
	char eth32Path[1024];
	strncpy_s(eth32Path, 1024, eth32.path, _TRUNCATE);
...
void CHook::UnreferenceBadPk3()
{
	...
	for (; browse; browse = browse->next)
	{
		...
		bool purePak = false;
		char buf[16];
		sprintf_s(buf,16,"%i", browse->pack->checksum);
...
void __cdecl eth32_etproAntiCheat(void *a, void *b, void *c, int checksum, void *e, char *orig_guid)
{
	...
	if (!IsBadReadPtr(orig_guid, 41) && orig_guid && orig_guid[0] && strlen(orig_guid)==40){
		...
		if (eth32.settings.etproOs)
			checksum = 0x30C3E725;	// for win32 et 2.60b
		else
			checksum = 0x395789A0;	// for linux
#ifdef ETH32_DEBUG
		Debug.Log("setting checksum to %X", checksum);
#endif
	} else
		checksum = 0x40040;
...
void __cdecl eth32_LoadTGA( const char *name, byte **pic, int *width, int *height)
{
	...
	{ 
		strcpy((char *)name, "gfx/eth32_logo_huge.tga");
	...
	{
		strcpy((char *)name, "gfx/eth32_logo.tga");
		orig_LoadTGA(name, pic, width, height);
