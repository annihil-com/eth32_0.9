...
void CGui::Init(void)
{
	...
	if (init) {
		...
		ChatMessage("^7ETH^332 ^7(^3c^7)2007 - cheatersutopia.com ^3& ^7nixcoders.org");
		return;	
	}
	...
	winPicker->AddWindow("Custom Hitbox", CAT_AIMBOT);
	...
	winPicker->AddWindow("GUI", CAT_VISUAL);
	winPicker->AddWindow("Portal", CAT_NETWORK);
	winPicker->AddWindow("IRC", CAT_NETWORK);
	winPicker->AddWindow("Esp", CAT_MISC);
...
CWindow *CGui::CreateWin(char *title, int x, int y, int w, int h, wintype_t wtype, viewtype_t vtype)
{
	...
	{
	case WIN_BALLISTIC:
		win = new CBallisticWindow(title, x, w, w, h);
		break;
	case WIN_CHAT:
