...
{
	if (sfx > 0)
		orig_syscall(CG_S_STARTLOCALSOUND, sfx, channelNum, 127);
}
...
void CSyscall::UI_ExecuteText(char *buf)
{
...
void CSyscall::CG_Print(const char *txtFmt, ...)
{
	...
	vsprintf_s(text, sizeof(text), txtFmt, arglist);
	va_end(arglist);
...
bool CSyscall::isKeyActionDown(char *action)
{
	int key1, key2;
	...
	for ( i = 0; *s; i++, s++ ) {
		...
		if (*s == '[' && (!strncmp(s, "[lon]", 5) || !strncmp(s, "[lof]", 5))) {
			...
			if (togloc) {
				...
				strncpy_s(temp, MAX_TOKEN_CHARS, buf + prev, i - prev);
				strcat_s(token, MAX_TOKEN_CHARS, CG_TranslateString(temp));
			}
			else {
				strncat_s(token, MAX_TOKEN_CHARS, buf + prev, i - prev);
			}
